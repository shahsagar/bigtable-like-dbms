 ...
 * unpackage {@literal <key,data> } pairs in a space-efficient manner.  That is, the
 * packaged result contains the key followed immediately by the data value;
 * No padding bytes are used (not even for alignment).
 * <p>
 * Furthermore, the {@literal BT<*>Page } classes need
 * not know anything about the possible AttrType values, since all
 * their processing of {@literal <key,data> } pairs is done by the functions
 * declared here.
 * <p>
 * In addition to packaging/unpacking of {@literal <key,value> } pairs, we
